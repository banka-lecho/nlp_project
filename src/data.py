...
DATA_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "film_review_argumentation.csv"
)
...
def load_dataset(path=DATA_PATH, redact="none"):
    if redact not in REDACTIONS:
        raise ValueError(
            f"неизвестный redact={redact}, доступно: {', '.join(REDACTIONS)}"
        )
    path = Path(path)
    if not path.exists():
    ...
    df["author"] = df["author"].fillna(
        pd.Series([f"__unknown_{i}" for i in df.index], index=df.index)
    )
...
def split_dataset(df, seed=42, group=None):
    ...
    if group is None:
        train, rest = train_test_split(
            df, test_size=0.3, stratify=df["arg_label"], random_state=seed
        )
        val, test = train_test_split(
            rest, test_size=0.5, stratify=rest["arg_label"], random_state=seed
        )
    else:
