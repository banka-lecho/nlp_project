...
def encode(texts, tokenizer, encoder, device, max_length, batch_size, pooling):
    ...
    for start in range(0, len(texts), batch_size):
        ...
        pooled = (
            mean_pool(hidden, batch["attention_mask"])
            if pooling == "mean"
            else hidden[:, 0]
        )
        ...
        if sys.stdout.isatty():
            print(
                f"\r  {min(start + batch_size, len(texts))}/{len(texts)}",
                end="",
                flush=True,
            )
    if sys.stdout.isatty():
...
def fit_probe(
    train_x, train_y, val_x, val_y, grid=(0.0003, 0.001, 0.003, 0.01, 0.1, 1.0, 10.0)
):
...
def parse_args():
    ...
    parser.add_argument(
        "--redact", nargs="+", choices=list(REDACTIONS), default=list(REDACTIONS)
    )
...
def main():
    ...
    for redact in args.redact:
        ...
        train, val, test = split_dataset(
            load_dataset(redact=redact), args.split_seed, args.group
        )
        ...
        for name, part in [("train", train), ("val", val), ("test", test)]:
            ...
            features[name] = (
                scaler.fit_transform(raw) if name == "train" else scaler.transform(raw)
            )
        for task in TASKS:
            probe, c = fit_probe(
                features["train"],
            ...
            print(
                f"  {task:<8} macro-F1={rows[-1]['macro-F1']:.4f}  QWK={rows[-1]['QWK']:.4f}  C={c}"
            )

    table = pd.DataFrame(rows)
    print()
    print(
        table.pivot(index="redact", columns="task", values="macro-F1")
        .reindex([r for r in REDACTIONS if r in args.redact])
        .to_string()
    )
